...
class LPSE(scrapy.Spider):
  name = 'lpse'
  start_urls = ['https://www.lpse.kemenkeu.go.id/eproc4']
  ...
  def parse(self, response):
    ...
    for i in range(1, len(rows)+1):
      ...
      if jml is not None and int(jml) > 0:
        ...
        for j in range(1, int(jml)+1):
          ...
          l.append('https://www.lpse.kemenkeu.go.id'+link)
      # else:
      #   print('not pass')
      # print("==============")
    ...
    data['link'] = l
    data['penyelenggara'] = "lpse kemenkeu"
    data.to_excel("lpse_kemenkeu.xlsx")